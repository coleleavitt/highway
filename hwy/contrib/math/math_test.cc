...
struct TestFastExpMinusOrZero {
  ...
  HWY_NOINLINE void operator()(T, D d) {
    if (sizeof(T) == 4) {
      ...
      // Float Subnormal Range: [-104.0, -87.0]
      TestMath<T, D>("FastExpMinusOrZeroSubnormal", std::exp,
                     CallFastExpMinusOrZero, d, static_cast<T>(-FLT_MAX),
                     static_cast<T>(-87.0), 1);
    } else {
      ...
      // Double Subnormal Range: [-744.0, -708.0]
      TestMath<T, D>("FastExpMinusOrZeroSubnormal", std::exp,
                     CallFastExpMinusOrZero, d, static_cast<T>(-DBL_MAX),
                     static_cast<T>(-708.0), 1);
    }
  }
};
